...
def main():
    ...
    for i in range (len(cases)):
        ...
        cur = f"SELECT database_id, corpus_id, key1, key2, key3, key4, year FROM LongWordsKeys WHERE database_id = {cases[i]}"
        ...
        print(merged)
        if len(merged) > 1:
            fin = open(f"FourLongestMatchesCorpus.txt", "a")
            fin.writelines(f"{paperid}\n")
            fin.close()
        if len(merged) > 1:
            fin = open(f"FourLongestMatchesAmount.txt", "a")
            fin.writelines(f"{len(merged)}\n")
            fin.close()
        if len(merged) > 1:
            fin = open(f"FourLongestMatchesCorpusMatches.txt", "a")
