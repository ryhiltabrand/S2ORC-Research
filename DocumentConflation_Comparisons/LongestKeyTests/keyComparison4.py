...
def main(count):
    ...
    for i in range (len(cases)):
        ...
        print(merged)
        if len(merged) > 1:
            fin = open(f"Results/Four/Set{count}Four_WordsMatchesCorpus.txt", "a")
            fin.writelines(f"{paperid}\n")
            fin.close()
        if len(merged) > 1:
            fin = open(f"Results/Four/Set{count}Four_WordsMMatchesAmount.txt", "a")
            fin.writelines(f"{len(merged)}\n")
            fin.close()
        if len(merged) > 1:
            fin = open(f"Results/Four/Set{count}Four_WordsMMatchesCorpusMatches.txt", "a")
